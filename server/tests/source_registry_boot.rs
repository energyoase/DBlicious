...
async fn data_entities_page_routes_via_registry_for_default_binding() {
    ...
    let direct = server::data::entities_page_raw("product", 1, 100, None, Default::default()).await;
    let routed = server::data::entities_page("product", 1, 100, None, Default::default()).await;
