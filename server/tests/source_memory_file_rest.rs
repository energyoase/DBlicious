...
async fn memory_source_read_only_rejects_mutates() {
    ...
    let err = src
        .create(&binding, None, serde_json::Map::new(), None)
        .await
        .unwrap_err();
...
async fn file_source_persists_to_disk_and_reloads() {
    ...
    // 1. Schreibphase: Source legt Datei + Inhalt an
    {
        ...
        src.create(&binding, Some("w-1".into()), fields, None)
            .await
            .unwrap();
    }
...
async fn file_source_init_on_missing_file_is_ok() {
    ...
    src.init()
        .await
        .expect("missing file ⇒ empty layout, kein Error");
    let binding = widgets_binding("disk", "widgets");
    let page = src
        .list_page(&binding, &PageQuery::default())
        .await
...
async fn rest_source_uses_rest_endpoint_locator() {
    ...
    let binding = EntityBinding {
        source: "api".into(),
        locator: BindingLocator::RestEndpoint {
            path: "my/path".into(),
        },
        primary_key: vec!["id".into()],
        read_only: false,
...
async fn boot_registry_recognises_memory_file_rest() {
    ...
    assert!(
        res.is_ok(),
        "Boot mit memory/file/rest muss klappen: {res:?}"
    );
