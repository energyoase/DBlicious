...
pub fn ColumnEditorPopover(
    ...
    let ov = move || {
        current_override
            .get()
            .unwrap_or_else(|| ViewPropertyOverride {
    ...
    let style = format!(
        "{card} position: absolute; top: {top}px; left: {left}px; width: 280px; z-index: 1000;"
    );
