...
fn entity_link_gets_designer_child() {
    ...
    assert_eq!(
        ...
        MenuAction::Link { route: "/builder/product".into() }
    );
}
...
fn group_recurses_into_children() {
    let tree = vec![group(
        "catalog",
        vec![link("products", "/entities/product"), link("categories", "/entities/category")],
    )];
    let out = augment_with_designer_links(tree);
    let catalog = &out[0];
    assert!(catalog.children.iter().any(|c| c.id == "products"));
    let products = catalog.children.iter().find(|c| c.id == "products").unwrap();
    assert_eq!(products.children.len(), 1);
    assert_eq!(products.children[0].id, "products__designer");
    let categories = catalog.children.iter().find(|c| c.id == "categories").unwrap();
