...
| 1.7.9 | PDF-Generierung via Template (typst preferred, fallback wkhtmltopdf); Templates im neuen `pdf_templates`-Eintrag oder per Designer pflegbar | M | `server/src/pdf/{mod,stub,typst}.rs`; Tests `server/tests/pdf_stub.rs` + `server/tests/pdf_typst.rs` (4) + Fixture `examples/shop/pdf-templates/invoice.typ`. PdfRenderer-Trait + StubRenderer + **TypstRenderer** (typst-as-lib 0.15.4 / typst 0.14.2, embedded Fonts via `typst-kit-embed-fonts`, kein eigenes TTF). Vars als JSON unter `sys.inputs.data` (kein String-Interpolations-Injection); Template liest `#import sys: inputs` + `json(bytes(inputs.data))`. PDF-Export via `typst-pdf`. Tests via `%PDF-`-Magic + `pdf-extract`-Text-Roundtrip + TemplateInvalid bei Syntaxfehler. Plan: [`docs/superpowers/plans/2026-05-23-1.7.9-typst-pdf-backend.md`](./docs/superpowers/plans/2026-05-23-1.7.9-typst-pdf-backend.md). pdf_templates-Tabelle + Designer-Anbindung bleiben Folge. | ✅ Stub + Typst (embedded fonts, sys.inputs-Vars); pdf_templates-Tabelle + Designer offen |
