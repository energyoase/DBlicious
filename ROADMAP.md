...
| 1.7.9 | PDF-Generierung via Template (typst preferred, fallback wkhtmltopdf); Templates im neuen `pdf_templates`-Eintrag oder per Designer pflegbar | M | `server/src/pdf/{mod,stub}.rs`; Tests `server/tests/pdf_stub.rs`. PdfRenderer-Trait + StubRenderer (ASCII-PDF mit `%PDF-`-Magic) heute; Typst-Backend als Folge-Item — typst-Crate ~30MB. **Implementierungsplan** liegt vor: [`docs/superpowers/plans/2026-05-23-1.7.9-typst-pdf-backend.md`](./docs/superpowers/plans/2026-05-23-1.7.9-typst-pdf-backend.md) (typst-as-lib + typst-assets embedded fonts, Vars via `sys.inputs`-JSON, Spike-First wegen API-Instabilitaet, CLI-Subprocess als Fallback). pdf_templates-Tabelle + Designer bleiben Folge. | 🟡 Trait + Stub; Typst-Backend geplant (Plan vorhanden) |
